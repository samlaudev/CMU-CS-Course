...
struct ltree {
  unsigned int leafcount;    // just to make example more interesting
  enum nodetype type;
  union nodecontent content;
};

// Checks that the leafcount of each node is correct
...
int main () {
  ...
  for (float res = 0.0; res != 5.0; res += 0.1 ) { // won't work!
    printf("res = %f\n", res);
  }
